    ...
    ULONG Size)
{
    ...
    if (!Buffer) {
        DbgPrint("Cannot display null buffer.\n");
        return;
    }

    DbgPrint("Displaying buffer at (0x%X)  Size (%d).\n", Buffer, Size);
 ...
 */
{
    ...
    DbgPrint("Irp->UserEvent = %p\n", Irp->UserEvent);

    Status = IoCallDriver(DeviceObject, Irp);
    DbgPrint("IoCallDriver: %08x\n", Status);

    if ((Status == STATUS_PENDING) && (Event != NULL)) {
        DbgPrint("Waiting on transport.\n");
 ...
 */
{
    ...
    DbgPrint("Called. DeviceName (%wZ, %u)\n", DeviceName, ShareExclusive);
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            DbgPrint("Got handle (%p)  Object (%p)\n", *Handle, *Object);
        }
    } else {
 ...
 */
{
    ...
    DbgPrint("Called. DeviceName (%wZ)  Name (%p)\n", DeviceName, Name);
    ...
    Status = TdiOpenDevice(DeviceName,
                           ...
                           AddressHandle,
                           AddressObject);
    ExFreePoolWithTag(EaInfo, TAG_NETIO_EA_INFO);
    return Status;
}
    ...
    PUINT MaxDatagramLength)
{
    ...
    if (!Mdl)
    {
        ExFreePoolWithTag(Buffer, TAG_NETIO_DATA_BUFFER);
 ...
 */
{
    ...
    if (!ConnectionObject) {
        DbgPrint("Bad connection object.\n");
        return STATUS_INVALID_PARAMETER;
    }

    DeviceObject = IoGetRelatedDeviceObject(ConnectionObject);
    if (!DeviceObject) {
        DbgPrint("Bad device object.\n");
        return STATUS_INVALID_PARAMETER;
    }
 ...
 */
{
    ...
    DbgPrint("Called. AddressHandle (%p)  ConnectionObject (%p)\n", AddressHandle, ConnectionObject);
 ...
 */
{
    ...
    DbgPrint("Called. ConnectionObject (%p)\n", ConnectionObject);
 ...
 */
{
    ...
    if (!NT_SUCCESS(Status)) {
        DbgPrint("Unable to get list of supported entities (Status = 0x%X).\n", Status);
        ExFreePoolWithTag(Entities, TAG_NETIO_TRANSPORT_ADDRESS);
        return Status;
    }
    ...
    for (i = 0; i < EntityCount; i++) {
        if (Entities[i].tei_entity == CL_NL_ENTITY) {
            ...
            if (!NT_SUCCESS(Status) || (EntityType != CL_NL_IP)) {
                DbgPrint("Unable to get entity of type IP (Status = 0x%X).\n", Status);
                break;
            }
            ...
            if (!NT_SUCCESS(Status) || (SnmpInfo.ipsi_numaddr == 0)) {
                DbgPrint("Unable to get SNMP information or no IP addresses available (Status = 0x%X).\n", Status);
                break;
            }
            ...
            if (SnmpInfo.ipsi_numaddr != 0) {
                ...
                Status = TdiQueryInformationEx(FileObject,                  /* File object */
                                               ...
                                               &BufferSize);                /* Output buffer size */
                if (!NT_SUCCESS(Status)) {
                    DbgPrint("Unable to get IP address (Status = 0x%X).\n", Status);
                    ExFreePoolWithTag(IpAddress, TAG_NETIO_SNMP_ADDRESS_INFO);
                    break;
                }
    ...
    PVOID CompletionContext)
{
    ...
    *Irp = TdiBuildInternalDeviceControlIrp(TDI_SEND,                /* Sub function */
    ...
    DbgPrint("Allocating irp for %p:%u\n", Buffer,BufferLength);
    ...
    PVOID CompletionContext)
{
    ...
    *Irp = TdiBuildInternalDeviceControlIrp(TDI_RECEIVE,             /* Sub function */
    ...
    _SEH2_TRY {
        DbgPrint("probe and lock\n");
        MmProbeAndLockPages(Mdl, (*Irp)->RequestorMode, IoModifyAccess);
        DbgPrint("probe and lock done\n");
    } _SEH2_EXCEPT(EXCEPTION_EXECUTE_HANDLER) {
        DbgPrint("MmProbeAndLockPages() failed.\n");
        IoFreeMdl(Mdl);
        IoCompleteRequest(*Irp, IO_NO_INCREMENT);
 ...
 */
{
    ...
    DbgPrint("Called(TransportObject %p)\n", TransportObject);
    ...
    if (BufferLength == 0)
    {
        DbgPrint("Succeeding send with length 0.\n");
        return STATUS_SUCCESS;
    }
