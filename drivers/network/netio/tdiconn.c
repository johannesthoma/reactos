...
PTRANSPORT_ADDRESS TaBuildNullTransportAddress(UINT AddressType)
{
    ...
    A = ExAllocatePoolWithTag(NonPagedPool, AddrLen, TAG_NETIO_TRANSPORT_ADDRESS);

    if (A)
    {
        if (TdiBuildNullTransportAddressInPlace(A, AddressType) != STATUS_SUCCESS)
        {
            ExFreePoolWithTag(A, TAG_NETIO_TRANSPORT_ADDRESS);
            return NULL;
        }
    }
 ...
 */
{
    ...
    ConnInfo = (PTDI_CONNECTION_INFORMATION)
        ExAllocatePoolWithTag(NonPagedPool,
                              ...
                              TAG_NETIO_TDI_CONNECTION_INFORMATION);
    if (!ConnInfo) {
        *ConnectionInfo = NULL;
    ...
    {
        ExFreePoolWithTag(ConnInfo, TAG_NETIO_TDI_CONNECTION_INFORMATION);
        ConnInfo = NULL;
    }
